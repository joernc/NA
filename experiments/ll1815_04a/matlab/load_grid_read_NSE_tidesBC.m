...
[am_u_S,ph_u_S,h_u_S,cl_u_S] = tmd_extract_HC('DATA_tpxo8_compact/Model_atlas_v1',y_u_S,x_u_S,'u');
...
[am_v_E,ph_v_E]= tidalConversionCorrection(startdate, am_v_E, ph_v_E, cl_v_E);
[am_u_E,ph_u_E]= tidalConversionCorrection(startdate, am_u_E, ph_u_E, cl_u_E);
...
am_v_E(isnan(am_v_E)) = 0;
am_u_E(isnan(am_u_E)) = 0;

ph_v_N(isnan(ph_v_N)) = 0;
ph_u_N(isnan(ph_u_N)) = 0;
ph_v_S(isnan(ph_v_S)) = 0;
ph_u_S(isnan(ph_u_S)) = 0;
ph_v_E(isnan(ph_v_E)) = 0;
ph_u_E(isnan(ph_u_E)) = 0;
...
filename = sprintf('OBEuam_%dx%d.bin', n0, nconst);
file = fopen([outdir, filename], 'w', 'ieee-be');
fwrite(file, am_u_E', 'real*4');
fclose(file);

filename = sprintf('OBEvam_%dx%d.bin', n0, nconst);
file = fopen([outdir, filename], 'w', 'ieee-be');
fwrite(file, am_v_E', 'real*4');
fclose(file);
...
filename = sprintf('OBEuph_%dx%d.bin', n0, nconst);
file = fopen([outdir, filename], 'w', 'ieee-be');
fwrite(file, ph_u_E', 'real*4');
fclose(file);

filename = sprintf('OBEvph_%dx%d.bin', n0, nconst);
file = fopen([outdir, filename], 'w', 'ieee-be');
fwrite(file, ph_v_E', 'real*4');
fclose(file);
